...
import restic_subset_calculator.cli
...
def test_run_restic_success(mocker):
    ...
    result, size = restic_subset_calculator.cli.run_restic(["list", "index"])
...
def test_run_restic_debug(mocker, capsys):
    ...
    restic_subset_calculator.cli.run_restic(["list", "index"], debug=True)
...
def test_run_restic_failure(mocker):
    ...
    mock_run.side_effect = subprocess.CalledProcessError(
        returncode=1, cmd="restic", stderr=b"some error"
    )

    with pytest.raises(SystemExit) as e:
        restic_subset_calculator.cli.run_restic(["list", "index"])

    assert e.value.code == 1


def test_main_invalid_t(mocker):
    mocker.patch("sys.argv", ["restic_subset_calculator", "0"])
    with pytest.raises(SystemExit) as e:
        restic_subset_calculator.cli.main()
    assert e.value.code == 1

    mocker.patch("sys.argv", ["restic_subset_calculator", "257"])
    with pytest.raises(SystemExit) as e:
        restic_subset_calculator.cli.main()
    assert e.value.code == 1
...
def test_main_success(mocker, capsys):
    ...
    mocker.patch(
        "restic_subset_calculator.cli.run_restic",
        side_effect=[
    ...
    mocker.patch("sys.argv", ["restic_subset_calculator", "2"])
...
def test_main_complex_index_and_duplicate_packs(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic_subset_calculator", "1"])
...
def test_main_alignment(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic_subset_calculator", "10"])
    restic_subset_calculator.cli.main()
    captured = capsys.readouterr()
...
def test_main_debug_flag(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic_subset_calculator", "1", "--debug"])
    ...
    with patch(
        "restic_subset_calculator.cli.run_restic",
        wraps=restic_subset_calculator.cli.run_restic,
    ) as mock_run:
        restic_subset_calculator.cli.main()
...
def test_entry_point(mocker):
    ...
    runpy.run_path("src/restic_subset_calculator/cli.py", run_name="__main__")


def test_parse_json_output_empty():
    assert restic_subset_calculator.cli.parse_json_output("") == []


def test_parse_json_output_ndjson():
    ndjson = '{"a":1}\n{"b":2}'
    assert restic_subset_calculator.cli.parse_json_output(ndjson) == [{"a": 1}, {"b": 2}]
