...
import restic_tools.subset_calculator
...
def test_run_restic_success(mocker):
    ...
    result, size = restic_tools.subset_calculator.run_restic(["list", "index"])
...
def test_run_restic_debug(mocker, capsys):
    ...
    restic_tools.subset_calculator.run_restic(["list", "index"], debug=True)
...
def test_run_restic_failure(mocker):
    ...
    with pytest.raises(SystemExit) as e:
        restic_tools.subset_calculator.run_restic(["list", "index"])

    assert e.value.code == 1


def test_main_invalid_t(mocker):
    mocker.patch("sys.argv", ["restic-subset-calculator", "0"])
    with pytest.raises(SystemExit) as e:
        restic_tools.subset_calculator.main()
    assert e.value.code == 1

    mocker.patch("sys.argv", ["restic-subset-calculator", "257"])
    with pytest.raises(SystemExit) as e:
        restic_tools.subset_calculator.main()
    assert e.value.code == 1
...
def test_main_success(mocker, capsys):
    ...
    mocker.patch(
        "restic_tools.subset_calculator.run_restic",
        side_effect=[
    ...
    mocker.patch("sys.argv", ["restic-subset-calculator", "2"])
...
def test_main_complex_index_and_duplicate_packs(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic-subset-calculator", "1"])
...
def test_main_alignment(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic-subset-calculator", "10"])
    restic_tools.subset_calculator.main()
    captured = capsys.readouterr()
...
def test_main_debug_flag(mocker, capsys):
    ...
    mocker.patch("sys.argv", ["restic-subset-calculator", "1", "--debug"])
    ...
    with patch(
        "restic_tools.subset_calculator.run_restic",
        wraps=restic_tools.subset_calculator.run_restic,
    ) as mock_run:
        restic_tools.subset_calculator.main()
...
def test_entry_point(mocker):
    ...
    runpy.run_path("src/restic_tools/subset_calculator.py", run_name="__main__")


def test_parse_json_output_empty():
    assert restic_tools.subset_calculator.parse_json_output("") == []
